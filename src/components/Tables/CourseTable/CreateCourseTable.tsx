...
export default function CreateOrEditCourseForm({
  courseData = {},
  id,
}: {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      let response;
      if (id) {
        response = await axios.put(`/api/courses/${id}`, formDataToSubmit, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Course updated successfully");
      } else {
        response = await axios.post(`/api/courses`, formDataToSubmit, {
          headers: { "Content-Type": "multipart/form-data" },
        });
